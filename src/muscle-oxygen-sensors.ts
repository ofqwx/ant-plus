...
function updateState(
  ...
  switch (page) {
    case 0x01: {
      ...
      const capabilities = data.readUInt16LE(
        Messages.BUFFER_INDEX_MSG_DATA + 3
      );
      const total =
        data.readUInt16LE(Messages.BUFFER_INDEX_MSG_DATA + 4) & 0xfff;
      const previous =
        (data.readUInt16LE(Messages.BUFFER_INDEX_MSG_DATA + 5) >> 4) & 0x3ff;
      const current =
        (data.readUInt16LE(Messages.BUFFER_INDEX_MSG_DATA + 6) >> 6) & 0x3ff;
    ...
    case 0x52: {
      ...
      const operatingTime =
        data.readUInt32LE(Messages.BUFFER_INDEX_MSG_DATA + 3) & 0xffffff;
      const batteryFrac = data.readInt32LE(Messages.BUFFER_INDEX_MSG_DATA + 6);
      const batteryStatus = data.readInt32LE(
